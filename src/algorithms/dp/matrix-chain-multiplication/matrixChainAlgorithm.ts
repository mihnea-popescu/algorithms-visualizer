...
export function matrixChainSteps(dimensions: number[]): Step[] {
  ...
  for (let l = 2; l <= n; l++) {
    ...
    for (let i = 0; i <= n - l; i++) {
      ...
      for (let k = i; k < j; k++) {
        const cost =
          m[i][k] + m[k + 1][j] + dimensions[i] * dimensions[k + 1] * dimensions[j + 1];
      ...
      const explanation = `Computing m[${i}][${j}]: Try splits at k = ${i} to ${j - 1}. ` +
        `Optimal split at k = ${optimalK}: ` +
        `m[${i}][${optimalK}] + m[${optimalK + 1}][${j}] + ` +
        `${dimensions[i]}×${dimensions[optimalK + 1]}×${dimensions[j + 1]} = ` +
        `${m[i][optimalK]} + ${m[optimalK + 1][j]} + ${dimensions[i] * dimensions[optimalK + 1] * dimensions[j + 1]} = ${m[i][j]}`;
